...
T_CHAMBER = 1350.0          # Chamber temperature [K]
                            # Assumption: Tc ≈ 900 + 600*alpha for alpha=0.5
...
NOZZLE_EFFICIENCY = 0.95    # Overall nozzle efficiency (dimensionless)
                            # Assumption: Accounts for divergence, boundary layer, and
                            # other real-world losses vs ideal expansion
                            # Typical range: 0.95-0.98 for well-designed nozzles
...
DESIGN_FEED_PRESSURE = 0.25     # MPa — selected operating feed pressure
                                 # Rationale: Mid-point of 0.15-0.30 MPa range
                                 # Provides margin on both ends for blowdown operation
...
def main():
    ...
    # Step 3: Compute mass flow rate required for target thrust
    # From F = mdot * Ve (vacuum, ignoring Pe*Ae term for initial sizing)
    # But we need Ve first, which depends on expansion ratio...

    # First, compute isentropic properties for given expansion ratio
    ...
    # Step 4: Compute required mass flow rate
    # F = mdot * V_e (vacuum, ignoring Pe*Ae term - it's small for vacuum operation)
    mdot_required = TARGET_THRUST / V_e_actual
    print(f"\nRequired mass flow rate: {mdot_required:.6f} kg/s")

    # Step 5: Compute throat area and diameter
    # mdot = Pc * At / c*
    At = mdot_required * c_star / P_c
    ...
    # Step 8: Compute thrust and Isp with Pe*Ae term included
    ...
    # Write JSON output
    with open("design/data/performance.json", "w") as f:
        json.dump(output, f, indent=2, default=str)

    print(f"\n✓ Design data written to design/data/performance.json")
...
def generate_performance_plot(Pc_MPa, thrust_N, Isp_s, expansion_ratio):
    ...
    for Pc_MPa in Pc_range:
        ...
        # Required mdot for 1.0 N target
        mdot = 1.0 / V_e_actual
